...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func directlyProcessHotkeySelection(_ text: String) {
        ...
        // 確保所有 UI 操作在主線程執行
        Task { @MainActor in
            logger.info("[熱鍵觸發] 開始在主線程上執行UI操作")
            
            // 更新 AppState 的原始文本
            AppState.shared.originalText = text
            logger.info("[熱鍵觸發] 已更新AppState的原始文本")
            
            // 更新 AppDelegate 的原始文本
            self.originalText = text
            logger.info("[熱鍵觸發] 已更新AppDelegate的原始文本")
            
            // 顯示文本窗口
            if let textWindowManager = self.textWindowManager {
                logger.info("[熱鍵觸發] 使用TextWindowManager顯示文本窗口")
                textWindowManager.showTextWindow(text: text)
            } else {
                logger.info("[熱鍵觸發] TextWindowManager不存在，使用showSwiftUITextWindow替代")
                showSwiftUITextWindow(text: text)
            }
            
            // 使用與懸浮按鈕相同的方法處理文本，避免不一致
            logger.info("[熱鍵觸發] 使用rewriteText方法處理")
            rewriteText()
            logger.info("[熱鍵觸發] 已調用rewriteText方法")
        }
    }
