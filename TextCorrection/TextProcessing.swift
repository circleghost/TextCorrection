...
class TextProcessing: @unchecked Sendable {
    ...
    static func diffStrings(_ old: String, _ new: String) -> [DiffChange] {
        ...
        // 逐行比較
        let minLineCount = min(oldLines.count, newLines.count)
        for i in 0..<minLineCount {
            ...
            // 相同行直接添加為equal
            if oldLine == newLine {
                result.append(.equal(oldLine))
            } else {
                // 對每行進行字元級別的比較
                let lineDiff = diffStringsByCharacter(oldLine, newLine)
                result.append(contentsOf: lineDiff)
            }
            
            // 如果不是最後一行，添加換行符
            if i < minLineCount - 1 || (oldLines.count > minLineCount && newLines.count > minLineCount) {
                result.append(.equal("\n"))
            }
        }
        
        // 處理剩餘的行
        if oldLines.count > minLineCount {
            for i in minLineCount..<oldLines.count {
                if !oldLines[i].isEmpty {
                    result.append(.delete(oldLines[i]))
                }
                if i < oldLines.count - 1 {
                    result.append(.delete("\n"))
                }
            }
        } else if newLines.count > minLineCount {
            for i in minLineCount..<newLines.count {
                if !newLines[i].isEmpty {
                    result.append(.insert(newLines[i]))
                }
